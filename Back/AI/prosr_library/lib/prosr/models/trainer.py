...
class SimultaneousMultiscaleTrainer(object):
    ...
    def get_current_errors(self):
        d = OrderedDict()
        # self.opt.train.l1_loss_weight = 1.0
        for s in self.opt.data.scale:
            if self.opt.train.l1_loss_weight > 0:
                d['l1_x%d' % s] = np.nan

        if hasattr(self, 'l1_loss'):
            print("hasattr진입!!", self.l1_loss.item()," model_scale: ",self.model_scale)
...
class CurriculumLearningTrainer(SimultaneousMultiscaleTrainer):
    ...
    def reset_curriculum_for_dataloader(self):
        ...
        print("reset_curriculum_for_dataloader진입~~")
        print("growing_steps: ",len(self.opt.train.growing_steps))
        print("data_scale: ",len(self.opt.data.scale)*2-1)
        assert len(
